...
class EnsembleDynamicsModel():
    ...
    def train(self, inputs, labels, tv_weights=None, batch_size=256, holdout_ratio=0.,
              max_epochs_since_update=5, logger=None):
        ...
        if tv_weights is not None:
            tv_weights = tv_weights[permutation]
            train_tv_weights = tv_weights[num_holdout:]
        ...
        for epoch in itertools.count():
            ...
            for start_pos in range(0, train_inputs.shape[0], batch_size):
                if tv_weights is not None:
                    idx = np.vstack(
                        [np.array(list(WeightedRandomSampler(train_tv_weights, batch_size, replacement=True)))
    ...
    def predict(self, inputs, batch_size=1024, factored=True):
        ...
        if factored:
            ...
            var = torch.mean(ensemble_var, dim=0) + torch.mean(torch.square(ensemble_mean - mean[None, :, :]),
                                                               dim=0)
            return mean, var
